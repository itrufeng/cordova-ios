...
-(void)drawRect:(CGRect)rect
{
    ...
    for (int i = 0 ; i < self.snsNames.count ; i++) {
        ...
        if (snsNamelabel == nil) {
            UILabel *snsNamelabel2 = [[UILabel alloc] initWithFrame:labelRect];
            snsNamelabel2.tag = snsPlatform.shareToType;
            snsNamelabel2.textAlignment = UITextAlignmentCenter;
            [snsNamelabel2 setBackgroundColor:[UIColor clearColor]];
            [snsNamelabel2 setTextColor:[UIColor whiteColor]];
            [snsNamelabel2 setFont:[UIFont systemFontOfSize:12]];
            [snsNamelabel2 setText:snsDisplayName];
            [_actionSheetBackground addSubview:snsNamelabel2];
        ...
        if (snsButton == nil) {
            UIButton *snsButton2 = [UIButton buttonWithType:UIButtonTypeCustom];
            UIImage *snsImage = [UIImage imageNamed:snsPlatform.bigImageName];
            [snsButton2 setBackgroundImage:snsImage forState:UIControlStateNormal];
            snsButton2.frame = buttonRect;
            snsButton2.tag = snsPlatform.shareToType + 100;
            [snsButton2 addTarget:self action:@selector(actionToSnsButton:) forControlEvents:UIControlEventTouchUpInside];
            [_actionSheetBackground addSubview:snsButton2];
        }
        else{
